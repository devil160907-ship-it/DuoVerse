...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'duoverse-secret-key-change-in-production'
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///database.db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['UPLOAD_FOLDER'] = 'static/uploads'
app.config['MAX_CONTENT_LENGTH'] = 16 * 1024 * 1024
app.config['YOUTUBE_API_KEY'] = 'AIzaSyCHPCJ_w-Qv6LVKulUB1NYfCvoJEYE4cQ4'
app.config['QR_FOLDER'] = 'static/qrcodes'

db = SQLAlchemy(app)

# Hardcoded gallery password
...
def chat(room_id):
    ...
    if not meeting:
        abort(404)
    return render_template('chat.html', meeting=meeting, room_id=room_id)
...
def capture(room_id):
    ...
    if not meeting:
        abort(404)
    return render_template('capture.html', meeting=meeting, room_id=room_id)
...
def gallery(room_id):
    ...
    if not meeting:
        abort(404)
    return render_template('gallery.html', meeting=meeting, room_id=room_id, hardcoded_password=HARDCODED_PASSWORD)
